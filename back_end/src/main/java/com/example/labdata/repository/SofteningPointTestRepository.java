...
public interface SofteningPointTestRepository extends JpaRepository<SofteningPointTest, Long> {
     ...
     * @return 软化点试验记录
     */
    @Query(value = "SELECT * FROM asphalt_softening_point_test WHERE task_id = :taskId", nativeQuery = true)
     ...
     * @return 软化点试验记录列表
     */
    @Query(value = "SELECT * FROM asphalt_softening_point_test WHERE experimenter = :experimenter", nativeQuery = true)
     ...
     * @return 软化点试验记录列表
     */
    @Query(value = "SELECT * FROM asphalt_softening_point_test WHERE CAST(softening_temperature AS DECIMAL) BETWEEN :minTemp AND :maxTemp", nativeQuery = true)
